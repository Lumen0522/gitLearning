...
{
    QPoint pt = center(row, col);    //保存待画棋子的中心像素点坐标
...
bool Board::red(int id)       //判断某id棋子是否是红棋
{
    return _s[id]._red;
}
QString Board::name(int id)   //得到棋子的名字String
{
    return _s[id].name();
}
...
{
    return _bRedTurn == _s[id]._red;                //第一处重查错误 判等
}
...
int Board::getStoneId(int row, int col) // 2018-9-10 16:48 getStoneId 函数出现问题
{
    for(int i=0;i<32;i++)
    {
